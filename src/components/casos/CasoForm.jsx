...
export default function CasoForm({ caso, onSubmit, onCancel, isSubmitting }) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2 md:col-span-2">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" onClick={adicionarParte} className="bg-green-600 hover:bg-green-700">
                  Adicionar
                </Button>
              </div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {formData.partes_envolvidas.map((parte, index) => (
                    <div key={index} className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm flex items-center gap-2">
                      {parte}
                      <button type="button" onClick={() => removerParte(index)} className="hover:text-blue-600">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
              ...
              <div className="flex gap-2 flex-wrap">
                {CATEGORIAS.map((cat) => (
                  <button key={cat} type="button" onClick={() => toggleCategoria(cat)}
                    className={`px-4 py-2 rounded-full text-sm font-medium border-2 transition-all ${(formData.categorias || []).includes(cat) ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-600 border-gray-300 hover:border-blue-400"}`}>
                    {cat}
                  </button>
                ))}
              </div>
