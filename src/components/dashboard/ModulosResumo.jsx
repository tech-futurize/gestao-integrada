  ...
  return (
    <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="text-base font-bold" style={{ color: "#26405d" }}>{title}</h3>
...
function StatusBadge({ label, color, bg }) {
  return (
    <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ backgroundColor: bg, color }}>{label}</span>
  );
}
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="space-y-3">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Indicadores</p>
                <div className="space-y-2">
                  {[
                    ...
                    <div key={k.label} className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: k.bg }}>
...
function ResumoContratos({ projetoId }) {
  ...
  const statusColors = { "Ativo": "#16a34a", "Em Revisão": "#f59e0b", "Encerrado": "#9ca3af", "Suspenso": "#ef4444" };
  const statusBg = { "Ativo": "#f0fdf4", "Em Revisão": "#fef3c7", "Encerrado": "#f9fafb", "Suspenso": "#fef2f2" };
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-3 gap-6">
              <div className="col-span-1">
                ...
                ) : <p className="text-xs text-gray-400 py-4">Nenhum contrato</p>}
              </div>
              <div className="col-span-2">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Últimos Contratos</p>
                {contratos.length > 0 ? (
                  <div className="space-y-2">
                    {contratos.slice(0, 5).map(c => (
                      <div key={c.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-100">
                        <div className="min-w-0 flex-1">
                          <div className="text-sm font-semibold text-gray-700 truncate">{c.objeto || c.numero}</div>
                          <div className="text-xs text-gray-400">{c.fornecedor} · R${((c.valor_total || 0) / 1e3).toFixed(0)}k</div>
                        </div>
                        <StatusBadge label={c.status} color={statusColors[c.status] || "#6b7280"} bg={statusBg[c.status] || "#f9fafb"} />
                      </div>
                    ))}
                  </div>
...
function ResumoSuprimentos({ projetoId }) {
  const { data: requisicoes = [], isLoading: lR } = useQuery({
    queryKey: ["req_dash", projetoId],
    queryFn: () => entities.RequisicaoCompra.filter({ projeto_id: projetoId }),
    enabled: !!projetoId,
  });
  const { data: cotacoes = [], isLoading: lC } = useQuery({
    queryKey: ["cot_dash", projetoId],
    queryFn: () => entities.Cotacao.filter({ projeto_id: projetoId }),
    enabled: !!projetoId,
  });

  const funil = [
    { name: "Requisições", value: requisicoes.length, fill: "#26405d" },
    { name: "Cotações Abertas", value: cotacoes.filter(c => c.status === "Aberta" || c.status === "Em Análise").length, fill: "#3b82f6" },
    { name: "Aprovadas", value: cotacoes.filter(c => c.status === "Aprovada").length, fill: "#16a34a" },
    { name: "Canceladas", value: [...requisicoes, ...cotacoes].filter(r => r.status === "Cancelada" || r.status === "Cancelado").length, fill: "#ef4444" },
  ];

  const statusColors = { "Rascunho": "#9ca3af", "Aprovada": "#16a34a", "Em Cotação": "#f59e0b", "Pedido Emitido": "#00a49a", "Recebido": "#3b82f6", "Cancelada": "#ef4444" };

  return (
    <Card className="border-0 shadow-md">
      {(lR || lC) ? <div className="p-6"><Skeleton className="h-64 w-full" /></div> : (
        <>
          <SectionHeader title="Suprimentos" icon={ShoppingCart} color="#00a49a" link="Suprimentos" subtitle="Requisições, cotações e pedidos de compra" />
          <CardContent className="p-6">
            <div className="grid grid-cols-4 gap-4 mb-6">
              <KpiCard label="Requisições" value={requisicoes.length} icon={ShoppingCart} color="#26405d" />
              <KpiCard label="Cotações" value={cotacoes.length} icon={FileText} color="#00a49a" />
              <KpiCard label="Aprovadas" value={cotacoes.filter(c => c.status === "Aprovada").length} icon={CheckCircle} color="#16a34a" />
              <KpiCard label="Pedidos Emitidos" value={requisicoes.filter(r => r.status === "Pedido Emitido").length} icon={ArrowRight} color="#3b82f6" />
            </div>
            <div className="grid grid-cols-3 gap-6">
              <div className="col-span-1">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Funil de Compras</p>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={funil} layout="vertical" margin={{ top: 0, right: 8, left: 10, bottom: 0 }}>
              ...
              <div className="col-span-2">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Últimas Requisições</p>
                  ...
                  <div className="space-y-2">
                    {requisicoes.slice(0, 5).map(r => (
                      <div key={r.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-100">
                        <div className="min-w-0 flex-1">
                          <div className="text-sm font-semibold text-gray-700 truncate">RC-{r.numero || r.id?.slice(-4)}</div>
                          <div className="text-xs text-gray-400">{r.solicitante} · {r.data_necessidade || "—"}</div>
                        </div>
                        <StatusBadge label={r.status} color={statusColors[r.status] || "#6b7280"} bg={statusColors[r.status] + "20" || "#f9fafb"} />
                      </div>
                    ))}
                  </div>
                ) : <p className="text-xs text-gray-400">Nenhuma requisição cadastrada</p>}
              </div>
            </div>
          </CardContent>
        </>
      ...
      {isLoading ? <div className="p-6"><Skeleton className="h-64 w-full" /></div> : (
        <>
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-3 gap-6">
              <div className="col-span-2">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Faturamento Mensal — Previsto vs. Realizado</p>
                {areaData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={200}>
                ...
                ) : <p className="text-xs text-gray-400 py-4">Nenhum dado financeiro registrado</p>}
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Acumulado</p>
                {areaData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={200}>
      ...
      {isLoading ? <div className="p-6"><Skeleton className="h-64 w-full" /></div> : (
        <>
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-3 gap-6">
              <div className="col-span-2">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Curva S — Avanço Acumulado</p>
                {areaData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={200}>
                    <AreaChart data={areaData} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
                      <defs>
                ...
                ) : <p className="text-xs text-gray-400 py-4">Nenhum dado de avanço registrado</p>}
              </div>
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Avanço Mensal</p>
                {areaData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={200}>
...
function ResumoPleitos({ projetoId }) {
  ...
  const statusColors = { "Aberto": "#ef4444", "Em Análise": "#f59e0b", "Em Andamento": "#3b82f6", "Resolvido": "#16a34a", "Fechado": "#9ca3af" };
  const statusBg = { "Aberto": "#fef2f2", "Em Análise": "#fef3c7", "Em Andamento": "#eff6ff", "Resolvido": "#f0fdf4", "Fechado": "#f9fafb" };
        ...
        <>
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-3 gap-6">
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Por Prioridade</p>
                {prioData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Últimos Pleitos</p>
                {casos.length > 0 ? (
                  <div className="space-y-2">
                    {casos.slice(0, 5).map(c => (
                      <div key={c.id} className="flex items-start justify-between p-3 rounded-lg bg-gray-50 border border-gray-100 gap-2">
                        <div className="min-w-0 flex-1">
                          <div className="text-xs font-semibold text-gray-700 truncate">{c.titulo}</div>
                          <div className="text-xs text-gray-400 mt-0.5">{c.data_abertura || "—"}</div>
                        </div>
                        <StatusBadge label={c.status} color={statusColors[c.status] || "#6b7280"} bg={statusBg[c.status] || "#f9fafb"} />
                      </div>
                    ))}
                  </div>
                ) : <p className="text-xs text-gray-400">Nenhum pleito cadastrado</p>}
              </div>
            </div>
          </CardContent>
        </>
...
function ResumoRegistros({ projetoId }) {
  ...
  const statusColors = { "Registrado": "#ef4444", "Em Análise": "#f59e0b", "Resolvido": "#3b82f6", "Fechado": "#16a34a" };
      ...
      {isLoading ? <div className="p-6"><Skeleton className="h-64 w-full" /></div> : (
        <>
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-3 gap-6">
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Por Tipo</p>
                <ResponsiveContainer width="100%" height={200}>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Por Gravidade</p>
                {gravData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={200}>
                    <PieChart>
              ...
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Últimos Registros</p>
                {incidentes.length > 0 ? (
                  <div className="space-y-2">
                    {incidentes.slice(0, 5).map(inc => (
                      <div key={inc.id} className="flex items-start justify-between p-3 rounded-lg bg-gray-50 border border-gray-100 gap-2">
                        <div className="min-w-0 flex-1">
                          <div className="text-xs font-semibold text-gray-700">{inc.tipo_registro}</div>
                          <div className="text-xs text-gray-400 truncate mt-0.5">{inc.descricao?.slice(0, 50) || "—"}</div>
                        </div>
                        <span className="text-xs font-semibold px-2 py-0.5 rounded-full flex-shrink-0"
                          style={{ backgroundColor: (statusColors[inc.status] || "#6b7280") + "20", color: statusColors[inc.status] || "#6b7280" }}>
                          {inc.status}
                        </span>
                      </div>
                    ))}
                  </div>
                ) : <p className="text-xs text-gray-400">Nenhum registro cadastrado</p>}
              </div>
            </div>
          </CardContent>
        </>
...
const nivelFromScore = (score) => {
  if (score >= 7) return { label: "Crítico", color: "#ef4444", bg: "#fef2f2" };
  if (score >= 4) return { label: "Alto", color: "#f59e0b", bg: "#fef3c7" };
  ...
  return (
    <Card className="border-0 shadow-md">
      ...
      <CardContent className="p-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Distribuição por Categoria</p>
            {riscosCAT.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
          ...
          <div className="col-span-2">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Riscos Críticos e Altos</p>
            {riscosAltos.length > 0 ? (
              <div className="space-y-2">
                {riscosAltos.map((r) => {
                  const nivel = nivelFromScore(r.score || 0);
                  return (
                    <div key={r.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-100 gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-semibold text-gray-700 truncate">{r.descricao || r.codigo}</div>
                        <div className="text-xs text-gray-400">Impacto: {r.impacto} · Prob.: {r.probabilidade}</div>
                      </div>
                      <StatusBadge label={nivel.label} color={nivel.color} bg={nivel.bg} />
                    </div>
                  );
                })}
              </div>
            ) : <p className="text-xs text-gray-400">Nenhum risco crítico ou alto</p>}
          </div>
        </div>
      </CardContent>
    </Card>
...
function ResumoPlanejamento({ projetoId }) {
  ...
  const { data: licoes = [] } = useQuery({
    queryKey: ["licoes_dash", projetoId],
    queryFn: () => entities.LicaoAprendida.filter({ projeto_id: projetoId }),
    enabled: !!projetoId,
  });
  ...
  return (
    <Card className="border-0 shadow-md">
      <SectionHeader title="Planejamento" icon={ClipboardList} color="#8b5cf6" link="Planejamento" subtitle="Atas, lições aprendidas, 6WLA e take-off" />
      <CardContent className="p-6">
        <div className="grid grid-cols-4 gap-4 mb-6">
          <KpiCard label="Atas de Reunião" value={atas.length} icon={ClipboardList} color="#26405d" />
          <KpiCard label="Lições Aprendidas" value={licoes.length} icon={CheckCircle} color="#00a49a" />
          <KpiCard label="PPC 6WLA" value={ppcMedio !== null ? `${ppcMedio}%` : "—"} icon={TrendingUp} color={ppcMedio !== null ? (ppcMedio >= 80 ? "#16a34a" : ppcMedio >= 60 ? "#d97706" : "#ef4444") : "#9ca3af"} />
          <KpiCard label="Itens Take-Off" value={commodities.length} icon={FileText} color="#8b5cf6" />
        </div>
        <div className="grid grid-cols-2 gap-6">
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Take-Off — Avanço por Disciplina</p>
              ...
              <div className="space-y-3 mt-1">
                {takeofProgresso.map(t => (
                  <div key={t.name}>
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">{(t.realizado || 0).toLocaleString("pt-BR")} / {(t.previsto || 0).toLocaleString("pt-BR")} {t.unidade}</div>
                  </div>
                ))}
              </div>
            ) : <p className="text-xs text-gray-400 py-4">Sem dados de take-off</p>}
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Avanço por Disciplina</p>
            {takeofProgresso.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
...
export default function ModulosResumo({ projetoId }) {
  return (
    <div className="space-y-5">
      <div className="pb-1">
        <h2 className="text-xl font-bold" style={{ color: "#26405d" }}>Resumo por Módulo</h2>
        <p className="text-sm text-gray-500">Visão consolidada de todos os módulos do projeto</p>
      </div>
