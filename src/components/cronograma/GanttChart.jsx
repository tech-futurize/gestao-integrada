...
export default function GanttChart({ tarefas, isLoading, zoom, showBaseline, showCritical, onView }) {
  ...
  const W_ID_VIS = idExpanded ? W_ID : 28;
  const W_FIXED  = W_ID_VIS + W_NIV + W_NOME + W_ACT;
  ...
  return (
    <Card className="bg-card shadow-sm flex flex-col overflow-hidden" style={{ height: "100%" }}>
      ...
      <div className="flex shrink-0 border-b border-border" style={{ height: HDR_H, background: BG_MUTED }}>
        ...
        <div className="flex shrink-0 border-r border-border" style={{ width: W_FIXED }}>
          <div className="flex items-center justify-between px-2 border-r border-border shrink-0" style={{ width: W_ID_VIS }}>
            {idExpanded && <span className="text-xs font-semibold text-muted-foreground">ID</span>}
            <button
              onClick={() => setIdExpanded(v => !v)}
              title={idExpanded ? "Ocultar coluna ID" : "Exibir coluna ID"}
      ...
      */}
      ...
      >
        ...
        <div className="shrink-0 border-r border-border relative"
             style={{ width: W_FIXED, height: totalH }}>
          {virtualRows.map(vr => {
            ...
            return (
              <div key={t.id}
                ...
                <div className="flex items-center border-r border-border shrink-0 pl-2"
                     style={{ width: W_ID_VIS, background: bg }}>
                  {idExpanded && (
                    <span title={t.codigo_wbs || "—"} className="text-xs text-muted-foreground font-mono truncate">
                      {t.codigo_wbs || "—"}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1 border-r border-border shrink-0 overflow-hidden"
                  ...
                  <span title={t.nome} className={`text-xs truncate ${t.tipo === "Resumo" ? "font-bold" : ""} ${isCritical ? "text-status-critical" : "text-foreground"}`}>
                    {t.nome}
                  </span>
                </div>
                {/* Ações */}
      ...
      <div className="flex shrink-0 border-t border-border">
        <div className="shrink-0" style={{ width: W_FIXED }} />
        {colsExpanded && (
          <div
