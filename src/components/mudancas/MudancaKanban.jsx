...
function MudancaCard({ mudanca, onEdit, onDelete, onUpdateStatus }) {
  ...
  return (
    <div
      className="bg-white rounded-xl shadow-sm p-3.5 space-y-2.5 hover:shadow-md transition-all duration-150 group"
      style={{ borderLeft: `3px solid ${stageConfig?.color || '#e5e7eb'}`, border: `1px solid #e5e7eb`, borderLeftWidth: 3, borderLeftColor: stageConfig?.color || '#e5e7eb' }}
    >
      <div className="flex items-start justify-between gap-2">
        <p className="text-sm font-semibold text-gray-800 leading-snug flex-1">{mudanca.titulo}</p>
        {mudanca.origem && (
      ...
      {mudanca.categorias?.length > 0 && (
        <div className="flex gap-1 flex-wrap">
          {mudanca.categorias.map(cat => (
            <span key={cat} className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${categoriaColors[cat] || 'bg-gray-100 text-gray-600'}`}>
              {cat}
            </span>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 py-1.5 px-2.5 bg-gray-50 rounded-lg">
          ...
          {mudanca.impacto_prazo_dias != null && (
            <div className="flex items-center gap-1 text-xs">
              ...
              <span className={`font-semibold ${mudanca.impacto_prazo_dias > 0 ? "text-red-600" : mudanca.impacto_prazo_dias < 0 ? "text-green-600" : "text-gray-500"}`}>
      ...
      <div className="flex gap-4 overflow-x-auto pb-3">
        {STAGES.map((stage) => {
          ...
          return (
            <div
              key={stage.key}
              className="flex-shrink-0 w-60 flex flex-col rounded-xl overflow-hidden"
            ...
            >
              ...
              >
                ...
                {stageMudancas.length === 0 && (
                  <div
                    className="flex items-center justify-center h-20 border-2 border-dashed rounded-xl text-xs text-gray-400"
